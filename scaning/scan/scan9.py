...
class AdvancedRecon:
    def __init__(self, target):
        # Pastikan hanya hostname yang dipakai (misal: example.com)
        ...
        # Wordlist untuk subdomain dan directory brute force
    ...
    def zone_transfer(self):
        """Coba lakukan AXFR (zone transfer) pada setiap nameserver."""
    ...
    def get_sitemap(self):
        try:
            ...
            if resp.status_code == 200:
                ...
                # Kumpulkan URL dari sitemap.xml (tidak menangani nested sitemap)
    ...
    def get_http_headers_and_title(self):
        try:
            ...
            # Coba ambil meta generator (sering mengindikasikan CMS atau framework)
    ...
    def technology_detection(self):
        # Deteksi sederhana berdasarkan header dan meta generator
        tech_info = {}
    ...
    def port_scan(self):
        ...
        # Menggunakan ThreadPoolExecutor untuk scan paralel
        def scan_port(port):
            try:
                s = socket.socket()
                s.settimeout(1)
    ...
    def geolocation_lookup(self):
        # Menggunakan ipinfo.io untuk mendapatkan data geolokasi IP
        try:
            if not self.ip:
                self.ip = socket.gethostbyname(self.domain)
    ...
    def display_results(self):
        ...
        # WHOIS
        if self.whois_info:
            whois_str = "\n".join([f"{key}: {val}" for key, val in self.whois_info.items()])
        else:
    ...
    def full_scan(self):
        ...
        self.whois_lookup()
        self.dns_lookup()
        self.zone_transfer()          # Mencoba AXFR pada nameserver
        self.get_robots_txt()
        self.get_sitemap()
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        console.print("[red]Usage: python scan7.py <target_url>[/red]")
        sys.exit(1)
